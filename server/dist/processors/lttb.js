...
exports.lttbDownsample = lttbDownsample;
exports.prepareLTTBData = prepareLTTBData;
function lttbDownsample(data, threshold) {
    ...
    for (let i = 0; i < threshold - 2; i++) {
        ...
        for (let j = rangeOffs; j < rangeTo; j++) {
            ...
            const area = Math.abs((pointATimestamp - avgTimestamp) * (pointBValue - pointAValue) -
                (pointATimestamp - pointBTimestamp) * (avgValue - pointAValue)) * 0.5;
            if (area > maxArea) {
                maxArea = area;
                maxAreaPoint = j;
            }
...
function prepareLTTBData(rawData, maxPoints = 200) {
