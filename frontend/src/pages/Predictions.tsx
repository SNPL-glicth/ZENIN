...
const Predictions = (): React.ReactElement => {
  ...
  const {
    ...
    error,
    refetch
  } = useAsyncData<Prediction[]>(() => predictionService.getRecentPredictions(20).then(r => r.data));
  ...
  return (
    <div>
      <div className="mb-6 md:mb-8">
        ...
        <p className="text-gray-600 text-sm md:text-base">Predicciones de series temporales</p>
      </div>

      <div className="flex flex-wrap gap-2 mb-6">
        {(['all', 'high-risk', 'stable'] as const).map((key) => (
          <button
            key={key}
